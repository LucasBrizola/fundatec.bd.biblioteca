...
insert into venda values(null, 20190525,"500000",null,"02131038015",12345,55555);
insert into venda values(null, 20180710,"140000",null,"03004005020",45500,11111);
insert into venda values(null, 20181212,"100000",null,"34567890140",54321,44444);
insert into venda values(null, 20190525,"200000",null,"06566677761",54560,22222);
insert into venda values(null, 20190525,"360000",null,"04005006067",65432,33333);
...
-- 1.       Listar os proprietários e a quantidade de Imóveis que possui a venda na Imobiliária

select count(imovel.inscricao) as total_imoveis, proprietario.nome, imovel.inscricao from proprietario, imovel where proprietario.inscricao_imovel = imovel.inscricao group by proprietario.nome;

-- 2.       Lista dos corretores e total de comissão de cada um

select corretor.nome, sum(venda.comissao) as comissao_total from corretor,venda where corretor.creci = venda.creci_corretor group by corretor.nome;

-- 3.       Listar quais imóveis foram vendidos agrupados por corretor, informando o preço da venda, comissão e nome do proprietário e comprador

select  corretor.nome, venda.inscricao_imovel, venda.preco_venda, venda.comissao, proprietario.nome, comprador.nome
from venda, corretor, proprietario, comprador, imovel
where venda.inscricao_imovel = imovel.inscricao
and venda.creci_corretor = corretor.creci
and venda.cpf_comprador = comprador.cpf
and proprietario.inscricao_imovel = imovel.inscricao;

-- 4.       Listar a quantidade de venda por mês para cada corretor e o total de comissão do mês.
select corretor.nome from venda, corretor where venda.creci_corretor = corretor.creci;
select month(venda.data_venda) from venda;

select month(venda.data_venda) as mes,
corretor.nome as nome,
count(venda.id_venda) as vendas_do_mes,
sum(venda.comissao) as comissao_total_mes
from corretor, venda
where venda.creci_corretor = corretor.creci
group by month(data_venda), corretor.nome asc;
